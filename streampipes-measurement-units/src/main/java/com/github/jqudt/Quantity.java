...
public class Quantity {
  ...
  public Quantity convertTo(Unit newUnit) throws IllegalArgumentException, IllegalAccessException {
    ...
    if (!unit.getType().equals(newUnit.getType())) {
      LOG.error("The new unit does not have the same parent type (source: {}; target: {})", unit.getType(),
              newUnit.getType());
      throw new IllegalAccessException("The new unit does not have the same parent type (source: %s; target: %s)"
              .formatted(unit.getType(), newUnit.getType()));
    }
