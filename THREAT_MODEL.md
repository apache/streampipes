...
- **Authentication** of UI/REST access before privileged actions. Authentication
  ...
  OAuth2 login is optional. JWT authentication requires a subject, expiration,
  an active principal, and an explicitly supported signing algorithm. Configured
  RSA key errors do not fall back to HMAC. The default rule is
...
- **Setting initial-admin credentials and provisioning the shared service secret**
  and hardening the deployment beyond convenience defaults (§8). Reports against
  exposed shipped defaults are handled according to §9, not automatically dismissed.
...
| --- | --- | --- |
| Initial-admin / auth setup | `SP_INITIAL_ADMIN_EMAIL` (default `admin@streampipes.apache.org`), `SP_INITIAL_ADMIN_PASSWORD` (application default `admin`; **generated by the Compose installer**), `SP_INITIAL_SERVICE_USER_SECRET` (**required; no default**). The Compose `configure` helper generates a persistent random service secret and shares it with extensions through `SP_CLIENT_SECRET`. On first start, `AutoInstallation` validates the service secret before provisioning accounts. Existing bootstrap accounts using the former public default are migrated when a valid replacement is configured; authentication with the old default is always rejected. | Service secrets must be provisioned consistently to the backend and extensions and preserved across restarts. The Compose installer also generates the initial human-admin password, CouchDB password, InfluxDB token and admin password, and encryption passcode. Deployments using other tooling must supply their own credentials. |
