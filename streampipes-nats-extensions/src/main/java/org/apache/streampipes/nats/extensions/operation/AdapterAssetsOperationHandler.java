...
public class AdapterAssetsOperationHandler implements ExtensionBrokerOperationHandler {

  private static final String OPERATION = "ADAPTER_ASSETS";
  private static final String TOPIC_OPERATION_SEGMENT = "adapter-assets";

  private final AdapterAssetManagement adapterAssetManagement;

  public AdapterAssetsOperationHandler(AdapterAssetManagement adapterAssetManagement) {
  ...
  public ExtensionServiceBrokerResponseEnvelope handle(ExtensionServiceBrokerRequestEnvelope request,
    ...
    if (operationSegments.isEmpty() || !TOPIC_OPERATION_SEGMENT.equals(operationSegments.get(0))) {
      return ExtensionBrokerResponseFactory.badRequest(
          ...
          "InvalidTopic",
          "Could not resolve adapter asset request from topic " + context.topic()
      );
    }
    ...
    var assetsOpt = adapterAssetManagement.getAssets(appId);
    ...
    return ExtensionBrokerResponseFactory.okBytes(request.getRequestId(), assetsOpt.get());
  }
}
