...
public class TransformationScriptLanguageResource {
  ...
  public List<ScriptMetadata> getAll(@RequestBody AdapterDescription adapterDescription) {
    ...
    if (!matchingServices.isEmpty()) {
      return matchingServices.get(0)
          ...
          .filter(metadata -> languagesSupportedByCore
              .stream()
              .anyMatch(coreLanguage -> coreLanguage.language().equals(metadata.language()))
          )
          .toList();
    } else {
