...
public abstract class MigrateExtensionsResource<T extends VersionedNamedStreamPipesEntity, ExT extends IParameterExtractor, MmT extends IModelMigrator<T, ExT>>
        extends
  ...
  /**
   * Find and return the corresponding {@link IModelMigrator} instance within the registered migrators. This allows to
   * pass the corresponding model migrator to a {@link ModelMigratorConfig} which is exchanged between Core and
   * Extensions service.
   *
  ...
  public Optional<MmT> getMigrator(ModelMigratorConfig modelMigratorConfig) {
    return DeclarersSingleton.getInstance().getServiceDefinition().getMigrators().stream()
            .filter(modelMigrator -> modelMigrator.config().equals(modelMigratorConfig))
            .map(modelMigrator -> (MmT) modelMigrator).findFirst();
  }

  /**
   * Migrates a pipeline element instance based on the provided {@link MigrationRequest}. The outcome of the migration
   * is described in {@link MigrationResult}. The result is always part of the response. Independent, of the migration
   * outcome, the returned response always has OK as status code. It is the responsibility of the recipient to interpret
   * the migration result and act accordingly.
   *
  ...
  protected MigrationResult<T> handleMigration(MigrationRequest<T> migrationRequest) {
    ...
    LOG.info("Received migration request for pipeline element '{}' to migrate from version {} to {}",
            pipelineElementDescription.getElementId(), migrationConfig.fromVersion(), migrationConfig.toVersion());
    ...
    return MigrationResult.failure(pipelineElementDescription, String
            .format("The given migration config '%s' could not be mapped to a registered migrator.", migrationConfig));
  }
  ...
  protected MigrationResult<T> executeMigration(MmT migrator, T pipelineElementDescription) {
    ...
    try {
      ...
      return MigrationResult.failure(pipelineElementDescription, String.format(
              "Migration failed due to an unexpected exception: %s", StringUtils.join(e.getStackTrace(), "\n")));
    }
  }
