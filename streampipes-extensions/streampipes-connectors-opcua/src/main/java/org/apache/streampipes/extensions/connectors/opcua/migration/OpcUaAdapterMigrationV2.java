...
public class OpcUaAdapterMigrationV2 implements IAdapterMigrator {
  ...
  public ModelMigratorConfig config() {
    return new ModelMigratorConfig("org.apache.streampipes.connect.iiot.adapters.opcua", SpServiceTagPrefix.ADAPTER, 1,
            2);
  }
  ...
  private StaticPropertyAlternatives getOpcHostOrUrl(AdapterDescription adapterDescription) {
    return (StaticPropertyAlternatives) adapterDescription.getConfig().stream()
            .filter(config -> config.getInternalName().equals(OPC_HOST_OR_URL.name())).findFirst().orElseThrow();
  }

  private StaticPropertyGroup getOpcHost(StaticPropertyAlternatives staticPropertyAlternatives) {
    return (StaticPropertyGroup) staticPropertyAlternatives.getAlternatives().stream()
            .filter(alternative -> alternative.getInternalName().equals(OPC_HOST.name())).findFirst().orElseThrow()
            .getStaticProperty();
  }

  private FreeTextStaticProperty getOpcServerPort(StaticPropertyGroup staticPropertyGroup) {
    return (FreeTextStaticProperty) staticPropertyGroup.getStaticProperties().stream()
            .filter(property -> property.getInternalName().equals(OPC_SERVER_PORT.name())).findFirst().orElseThrow();
  }
}
