            ...
            .requiredAlternatives(Labels.withId(OPC_HOST_OR_URL),
                    ...
                    Alternatives.from(Labels.withId(OPC_HOST),
                            StaticProperties.group(Labels.withId(HOST_PORT),
                                    ...
                                    StaticProperties.integerFreeTextProperty(Labels.withId(OPC_SERVER_PORT)))))
            .requiredRuntimeResolvableTreeInput(Labels.withId(AVAILABLE_NODES.name()), dependsOn, true, adapterConfig);
  }

  public static List<String> getDependsOn(boolean adapterConfig) {
    return adapterConfig
            ? List.of(ADAPTER_TYPE.name(), ACCESS_MODE.name(), OPC_HOST_OR_URL.name())
            : List.of(ACCESS_MODE.name(), OPC_HOST_OR_URL.name());
  }
}
