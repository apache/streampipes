...
public class SwingingDoorTrendingFilter {
  ...
  /**
   * the maximum absolute difference the user set if the data's value is within compressionDeviation, it will be
   * compressed and discarded after compression, it will only store out of range (time, data) to form the trend
   */
  private final double compressionDeviation;

  /**
   * the minimum time distance between two stored data points if current point time to the last stored point time
   * distance <= compressionMinTimeInterval, current point will NOT be stored regardless of compression deviation
   */
  private final long compressionMinTimeInterval;

  /**
   * the maximum time distance between two stored data points if current point time to the last stored point time
   * distance >= compressionMaxTimeInterval, current point will be stored regardless of compression deviation
   */
  private final long compressionMaxTimeInterval;

  /**
   * isFirstValue is true when the encoder takes the first point or reset() when cur point's distance to the last stored
   * point's distance exceeds compressionMaxTimeInterval
   */
  private boolean isFirstValue = true;

  /**
   * the maximum curUpperSlope between the lastStoredPoint to the current point upperDoor can only open up
   */
  private double upperDoor = Integer.MIN_VALUE;

  /**
   * the minimum curLowerSlope between the lastStoredPoint to the current point lowerDoor can only open downward
   */
  private double lowerDoor = Integer.MAX_VALUE;

  /**
   * the last read time and value if upperDoor >= lowerDoor meaning out of compressionDeviation range, will store
   * lastReadPair
   */
  ...
  /**
   * if current point to the last stored point's time distance >= compressionMaxTimeInterval, will store current point
