...
public class StreamPipesCoreApplication extends StreamPipesServiceBase {
  ...
  private final ISpCoreConfigurationStorage coreConfigStorage = StorageDispatcher.INSTANCE.getNoSqlStore()
  ...
  public static void main(String[] args) {
    ...
    application.initialize(() -> List.of(new SpNatsProtocolFactory(), new SpKafkaProtocolFactory(),
            new SpMqttProtocolFactory(), new SpJmsProtocolFactory(), new SpPulsarProtocolFactory()));
  }
  ...
  public void init() {
    ...
    executorService.schedule(new PostStartupTask(getPipelineStorage()),
            env.getInitialHealthCheckDelayInMillis().getValueOrDefault(), TimeUnit.MILLISECONDS);

    scheduleHealthChecks(env.getHealthCheckIntervalInMillis().getValueOrDefault(), List.of(new ServiceHealthCheck(),
            new PipelineHealthCheck(),
            new AdapterHealthCheck(StorageDispatcher.INSTANCE.getNoSqlStore().getAdapterInstanceStorage(),
                    new AdapterMasterManagement(StorageDispatcher.INSTANCE.getNoSqlStore().getAdapterInstanceStorage(),
    ...
    logCheckExecutorService.scheduleAtFixedRate(new ExtensionsServiceLogExecutor(), logFetchInterval, logFetchInterval,
  ...
  private void scheduleHealthChecks(int healthCheckIntervalInMillis, List<Runnable> checks) {
    ...
    checks.forEach(check -> {
      ...
      healthCheckExecutorService.scheduleAtFixedRate(check, healthCheckIntervalInMillis, healthCheckIntervalInMillis,
              TimeUnit.MILLISECONDS);
    });
  }
  ...
  public void onExit() {
    ...
    List<Pipeline> pipelinesToStop = getAllPipelines().stream().filter(Pipeline::isRunning).toList();
