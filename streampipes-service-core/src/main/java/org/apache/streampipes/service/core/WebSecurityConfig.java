...
public class WebSecurityConfig {
  ...
  public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
    http.cors().and().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS).and().csrf().disable()
            ...
            .authenticationEntryPoint(new UnauthorizedRequestEntryPoint()).and().authorizeHttpRequests((authz) -> {
              try {
                authz.requestMatchers(UnauthenticatedInterfaces.get().stream().map(AntPathRequestMatcher::new).toList()
                        .toArray(new AntPathRequestMatcher[0])).permitAll().anyRequest().authenticated();

                if (env.getOAuthEnabled().getValueOrDefault()) {
                  LOG.info("Configuring OAuth authentication from environment variables");
                  authz.and().oauth2Login().authorizationEndpoint()
  ...
  protected final ClientRegistration.Builder getBuilder(String registrationId) {
    ...
    builder.redirectUri(String.format("%s/streampipes-backend/{action}/oauth2/code/{registrationId}",
