...
public class PropertySelector {
  ...
  private List<EventProperty> extractProperties(List<EventProperty> inputProperties, List<String> propertySelectors,
          String currentPropertyPointer, List<EventProperty> appendProperties) {
      ...
      for (String propertySelector : propertySelectors) {
        if (isInSelection(inputProperty, propertySelector, currentPropertyPointer)) {
          ...
          if (outputProperty instanceof EventPropertyNested) {
            ((EventPropertyNested) outputProperty).setEventProperties(
                    extractProperties(((EventPropertyNested) outputProperty).getEventProperties(), propertySelectors,
                            makeSelector(currentPropertyPointer, inputProperty.getRuntimeName()), appendProperties));
          }
          if (isPresent(outputProperty.getRuntimeName(), outputProperties, appendProperties)) {
            String newRuntimeName = createRuntimeName(outputProperty.getRuntimeName(), outputProperties,
                    appendProperties);
            propertyRenameRules.add(new PropertyRenameRule(
                    makeSelector(currentPropertyPointer, outputProperty.getRuntimeName()), newRuntimeName));
  ...
  private String createRuntimeName(String runtimeName, List<EventProperty> outputProperties,
  ...
  private Boolean isPresent(String runtimeName, List<EventProperty> outputProperties,
          List<EventProperty> appendProperties) {
  ...
  private boolean isInSelection(EventProperty inputProperty, String propertySelector, String currentPropertyPointer) {
    return (currentPropertyPointer + PROPERTY_DELIMITER + inputProperty.getRuntimeName()).equals(propertySelector);
  }
  ...
  public List<EventProperty> createPropertyList(final List<String> propertySelectors,
    ...
    outputProperties.addAll(extractProperties(PropertySelectorUtils.getProperties(firstSchema),
            getPropertySelectors(propertySelectors, FIRST_STREAM_ID_PREFIX), FIRST_STREAM_ID_PREFIX, appendProperties));
    outputProperties.addAll(extractProperties(PropertySelectorUtils.getProperties(secondSchema),
            getPropertySelectors(propertySelectors, SECOND_STREAM_ID_PREFIX), SECOND_STREAM_ID_PREFIX,
            appendProperties));

    outputProperties
            .addAll(appendProperties.stream().map(ep -> new Cloner().property(ep)).collect(Collectors.toList()));
