...
public abstract class AbstractMigrationManager {
  ...
  /**
   * Performs the actual migration of a pipeline element. This includes the communication with the extensions service
   * which runs the migration.
   *
   * @param pipelineElement
   *          pipeline element to be migrated
   * @param migrationConfig
   *          config of the migration to be performed
   * @param url
   *          url of the migration endpoint at the extensions service where the migration should be performed
   * @param <T>
   *          type of the processing element
   * @return result of the migration
   */
  protected <T extends VersionedNamedStreamPipesEntity> MigrationResult<T> performMigration(T pipelineElement,
          ModelMigratorConfig migrationConfig, String url) {
    ...
    try {
      ...
      var migrationResponse = ExtensionServiceExecutions.extServicePostRequest(url, serializedRequest).execute();
      ...
      return JacksonSerializer.getObjectMapper().readValue(migrationResponseString, typeReference);
   ...
   *          List of migrations to take in account
   * @param serviceUrl
   *          Url of the extension service that provides the migrations.
   */
  protected void updateDescriptions(List<ModelMigratorConfig> migrationConfigs, String serviceUrl) {
    migrationConfigs.stream().collect(
            ...
            Collectors.toMap(ModelMigratorConfig::targetAppId, Function.identity(),
   ...
   *
   * @param requestUrl
   *          URl that references the description to be updated at the extensions service.
   */
  protected void performUpdate(String requestUrl) {

    try {
      var entityPayload = ExtensionServiceExecutions.extServiceGetRequest(requestUrl).execute().returnContent()
              .asString();
      var updateResult = new TypeExtractor(entityPayload).getTypeVerifier().verifyAndUpdate();
      if (!updateResult.isSuccess()) {
        LOG.error("Updating the pipeline element description failed: {}",
                StringUtils.join(updateResult.getNotifications().stream().map(Notification::toString).toList(), "\n"));
      }
